...
WebPage::WebPage() : m_title(), m_link() {}

WebPage::WebPage(const string &doc) : m_doc(doc) {
  if (!doc.empty()) {
    XMLDocument xml;
...
int WebPage::getDocId() const { return m_id; }

string WebPage::getTitle() const { return m_title; }

string WebPage::getLink() const { return m_link; }

string WebPage::getDoc() const { return m_doc; }

string WebPage::getContent() const { return m_context; }
